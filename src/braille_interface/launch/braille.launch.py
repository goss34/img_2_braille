...
def generate_launch_description() -> LaunchDescription:
    ...
    actions.append(
        ExecuteProcess(
            cmd=['/workspaces/isaac_ros-dev/src/scripts/reset_realsense.sh'],
            shell=True,
            output='screen',
        )
    )
    ...
    actions.append(
        ExecuteProcess(
            cmd=['ros2', 'run', 'braille_interface', 'heightmap_generator_node'],
            output='screen',
        )
    )

    # braille_pipeline_node
    actions.append(
        ExecuteProcess(
            cmd=['ros2', 'run', 'braille_interface', 'braille_server_node'],
            output='screen',
        )
    )
    ...
    actions.append(
        ExecuteProcess(
            cmd=['ros2', 'run', 'braille_interface', 'publish_map_node'],
            output='screen',
        )
    )
    ...
    actions.append(
        ExecuteProcess(
            cmd=['ros2', 'run', 'braille_interface', 'image_to_braille_node'],
            output='screen',
        )
    )
