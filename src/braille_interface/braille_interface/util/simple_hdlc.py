...
class Frame(object):
    ...
    def checkCRC(self):
        ...
        calculated_crc = calcCRC(self.data)

        # Log both in hex for human-readable comparison
        # print(f"  → CRC check: expected={expected_crc.hex()} calculated={calculated_crc.hex()}")

        res = (expected_crc == calculated_crc)
        if not res:
            logger.warning('invalid crc %s != %s', expected_crc.hex(), calculated_crc.hex())
            self.error = True
        return res
